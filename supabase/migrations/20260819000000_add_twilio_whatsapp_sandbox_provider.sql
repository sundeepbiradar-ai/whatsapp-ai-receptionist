-- Phase 9: additive support for a temporary Twilio WhatsApp Sandbox test
-- provider, alongside the unchanged meta_whatsapp_cloud provider.
-- Provider remains an explicit allowed set; it is never unrestricted text.
-- Only the constraints/RPC actually needed by the Twilio sandbox inbound
-- test path are widened. Delivery-status correlation and the outbound retry
-- job system are intentionally left meta_whatsapp_cloud-only.

alter table public.organization_whatsapp_configs
  drop constraint organization_whatsapp_configs_provider_check;

alter table public.organization_whatsapp_configs
  add constraint organization_whatsapp_configs_provider_check
  check (provider in ('meta_whatsapp_cloud', 'twilio_whatsapp_sandbox'));

alter table public.messages
  drop constraint messages_provider_check;

alter table public.messages
  add constraint messages_provider_check
  check (provider is null or provider in ('meta_whatsapp_cloud', 'twilio_whatsapp_sandbox'));

comment on column public.organization_whatsapp_configs.provider is
  'Allowed providers: meta_whatsapp_cloud (production) or twilio_whatsapp_sandbox (temporary test adapter).';
comment on column public.messages.provider is
  'Origin provider for externally correlated messages; meta_whatsapp_cloud, twilio_whatsapp_sandbox, or null for legacy/internal messages.';

-- Reused, unchanged persistence boundary: only the allowed-provider set grows.
...
begin
  if target_provider not in ('meta_whatsapp_cloud', 'twilio_whatsapp_sandbox')
...
comment on function public.process_inbound_whatsapp_message(uuid, uuid, text, text, text, timestamptz, text) is
  'Service-only atomic persistence boundary for trusted normalized inbound WhatsApp text messages; now accepts meta_whatsapp_cloud or twilio_whatsapp_sandbox.';
